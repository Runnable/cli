...
var Upload = module.exports = {
      ...
      .spread(function (argsWithRepo, instance) {
        return fs.readFileAsync(path.resolve(process.cwd(), argsWithRepo.file))
          .then(function (file) {
            ...
            return Upload._recusivelyCreateDirectories(container, workingDir, dirname)
              .then(function () {
  ...
  _createDirectory: function (container, path, directoryName) {
    var opts = {
      name: directoryName,
      path: path,
      isDir: true
  ...
  _createFile: function (container, path, fileName, file) {
    var opts = {
      name: fileName,
      path: path,
      isDir: false,
  ...
  _recusivelyCreateDirectories: function (container, basePath, fullPath) {
    ...
    var allPaths = fullPath.split(path.sep)
    if (allPaths[allPaths.length - 1].length === 0) {
      allPaths.pop()
    }
