...
class AutoIsolation extends Runnable {
  static handleAutoIsolation (args) {
    if (!args.enable && !args.disable) {
      return AutoIsolation.list(args)
    }
    return Utils.getRepositoryAndMasterPodInstance(args)
            ...
            () => {
              ...
              return read({ prompt: prompt })
                .then((selection) => {
                  ...
                  if (!isNumber(index)) {
                    console.error('your selection was not a number. try again.')
                    return
                  }
                  // now that we know it's a number, this is safe
                  index = index - 1
